...
dt$LoanOriginationDate.month <- factor(dt$LoanOriginationDate.month,
                                            months)
ggplot(subset(dt,!(LoanOriginationDate.year %in% c("2005", "2014"))), aes(as.factor(LoanOriginationDate.year))) +
...
ggplot(dt, aes(x = CreditScoreRangeUpper)) + 
  geom_histogram(aes(y = ..count..), binwidth = 10, fill='#05DBF2',
        color='black', position="identity") + scale_y_sqrt() +
        labs(x= 'Upper Range of Credit Score', y='Number of borrowers', title="Distribution of Credit Score Range (Upper)")
summary(dt$CreditScoreRangeLower)
summary(dt$CreditScoreRangeUpper)
```
...
ggplot(dt_pr, aes(y = LenderYield*100, x = BorrowerRate*100, color = ProsperRatings)) +
  ...
  labs(x = 'Borrower Rate(%)', y = 'Lender Yield(%)', title = " Lender Yield vs. Borrower Rate") +
...
p3 = ggplot(subset(dt, !(IncomeRange %in% c( "Not displayed"))),
  ...
  coord_cartesian(ylim = c(1, 1000000), xlim = c(1,1800)) + 
  geom_smooth(method = lm) +
  labs(x= 'Monthly Loan Payment($USD)', y= 'Monthly Income($USD)', title = "Income vs. Loan Payment")
...
p4 = ggplot(subset(dt, !(IncomeRange %in% c( "Not displayed"))),
  ...
  scale_y_log10 () + scale_color_brewer(palette="Dark2") +
  geom_smooth(method = lm) +
  labs(x= 'Original Loan Amount($USD)', y= 'Monthly Loan Payment($USD)', title = "Loan Payment vs. Borrowed Amount")
