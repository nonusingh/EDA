...
```{r, Univariate_Plots, fig.width=6, fig.height=4}
...
ggplot(top_list,
  ...
  geom_bar(stat="identity", fill='#05DBF2', color='black') + coord_flip() +
  labs(y = "Number of Borrowers", 
       x = "Top Reasons(Numerical)", 
...
> Note that at the 10 year mark in the plot, or the beginning of time, Credit grade lines converge at borrower rate ~ 0.1. This could be due to fact that the rating system back then was not robust enough to assign interest rates based on ratings and hence this data will be excluded from our analysis next. We will also remove borrowers that had income range as not employed, not displayed or equal to 0 or have borrower rate >0.35 or have credit score lower than 400

```{r, echo = TRUE}
dt <- subset(dt, !(ProsperRating == ""))
dt <- subset(dt, !(IncomeRange %in% c("Not employed","$0","Not displayed")))
...
```{r echo=FALSE, Bivariate_Plots, fig.width=6, fig.height=6}
ggplot(dt, aes(ProsperRating, LoanOriginalAmount)) +
...
```{r, fig.width=6, fig.height=6}
ggplot(dt, aes(y = DebtToIncomeRatio, x= IncomeRange)) +
  geom_boxplot(aes(fill = IncomeRange)) + theme(axis.text.x = element_text(angle = 90)) + 
  stat_summary(fun.y=median, geom="line", aes(group=1)) +
    scale_y_continuous(limits =  quantile(dt$DebtToIncomeRatio, 
                                        probs = c(0.05, .95), 
                                        na.rm = TRUE)) +
...
p1 <- ggplot(data=dt, aes(x=BorrowerRate, y= Investors, colour=ProsperRatings)) +
  # draw points
  geom_point(size=1.5, alpha=.6) + labs(title = "Borrower Rate vs. Investors") +
  # draw ellipse lines
  geom_path(data=dt_ell, aes(x=x1, y=y1,colour=group), size=1, linetype=1)

# create the ggplot with points colored by group
p2 <- ggplot(data=dt, aes(x=LoanOriginalAmount, y= Investors, colour=ProsperRatings)) +
  # draw points
  geom_point(size=1.5, alpha=.6) + labs(title = "LoanOriginalAmount vs. Investors") +
  # draw ellipse lines
...
ggpairs(subdt[,4:13], 
 ...
 lower = list(continuous = wrap("points", alpha = 0.3, size=0.1), 
              combo = wrap("dot", alpha = 0.05, size=0.1) ),
 title = "First 10 numerical Columns")
```
...
```{r echo=FALSE, Plot_Three, fig.width=12, fig.height=6}
dt_ell2 <- data.frame()
# for each level in dt$IncomeRange
for(g in levels(dt$IncomeRange)){
  # create 100 points per variable around the mean of each group
  dt_ell2 <- rbind(dt_ell2, 
                  cbind(as.data.frame(with(dt[dt$IncomeRange==g,],
                        ...
                        as.data.frame(with(dt[dt$IncomeRange==g,],
...
p3 = ggplot(dt, aes(x = MonthlyLoanPayment, y = StatedMonthlyIncome)) +
  ...
  geom_path(data=dt_ell2, aes(x=x1, y=y1,colour=group), size=1, linetype=1)
...
p4 = ggplot(dt, aes(x = LoanOriginalAmount, y = MonthlyLoanPayment)) +
  ...
  geom_path(data=dt_ell2, aes(x=x2, y=y2,colour=group), size=1, linetype=1)
