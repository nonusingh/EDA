...
ggplot(subset(dt,!(LoanOriginationDate.year %in% c("2005", "2014"))),
  ...
  ylab('Number of Loans') + scale_fill_brewer(palette = "Greens")
```
...
pl1 <- ggplot(subset(dt, !(EmploymentStatus %in% c("Not available" , ""))),
  ...
  geom_bar(fill='#05DBF2', color='black') + coord_flip() +
...
ggpairs(subsdt[sample(1:nrow(subsdt), "100",
  	replace=FALSE),], upper = list(wrap = c(size = 10)),
  lower = list(continuous = "smooth", wrap = c(method = "loess", fill = "blue"))) +
...
plt3 <- ggplot(dt, aes(x = age_with_months, 
               fill = ProsperRatings)) + 
  geom_histogram() + labs(title = " Time trend by Prosper Rating")

plt4 <- ggplot(dt, aes(x = age_with_months, fill = CreditGrade)) + 
  geom_histogram() + labs(title = " Time trend by CreditGrade")
...
dt <- subset(dt, !(IncomeRange %in% c("Not displayed", "", "$0")))
...
ggplot(dt, aes(y = EstimatedEffectiveYield, x = BorrowerRate, 
    color = ProsperRatings)) + scale_colour_brewer(palette = "Greens") +
...
```{r, fig.width=6, fig.height=4}
ggplot(subset(dt, !(IncomeRanges =="Not employed")), 
...
> Yes, The model I created has Adjusted R2 of 0.9109. Strengths of the model is that it includes most of the contributing numerical factors affecting yield, the limitation is the assumption that all the factors are linear
